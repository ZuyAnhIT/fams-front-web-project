...
export const customerDashboardKeys = {
  ...
  hr: (tenantId: string) => [...customerDashboardKeys.all(tenantId), 'hr'] as const,
...
export function useHrDashboard(tenantId: string, enabled = true) {
  return useQuery({
    queryKey: customerDashboardKeys.hr(tenantId),
    queryFn: () => customerDashboardService.hr(tenantId),
    enabled: Boolean(tenantId) && enabled,
    staleTime: 30_000,
  });
}
