...
import { Building2, RefreshCw, ShieldAlert, UserMinus, Users } from 'lucide-react';
...
export default function SitePresenceReportTab() {
  ...
  const query = useSitePresenceReport(tenantId, { siteId, workspaceId, page, size: 20 });
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex flex-wrap gap-3">
        ...
        <BaseSelect className="min-w-56" aria-label="Lọc hiện diện theo workspace" allowClear placeholder="Tất cả workspace" value={workspaceId} onChange={(value) => { setWorkspaceId(value); setPage(0); }} options={(workspacePage?.data?.content ?? []).map((workspace) => ({ value: workspace.id, label: workspace.name }))} />
      </div>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <StatCard title="Vi phạm chưa xử lý" value={report?.totalUnresolvedViolations ?? 0} icon={ShieldAlert} tone="violet" loading={query.isLoading} />
    </div>
    <div className="grid gap-4 xl:grid-cols-2">{(report?.sites.content ?? []).map((site) => {
      ...
      return <section key={site.siteId} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-slate-900">{site.siteName}</h3><p className="mt-1 text-xs text-slate-500">{site.timezone}</p></div><div className="flex flex-col items-end gap-1"><Tag color={site.absentCount ? 'warning' : 'success'}>{site.presentCount}/{site.assignedCount} có mặt</Tag>{site.unresolvedViolations > 0 && <Tag color="error">{site.unresolvedViolations} vi phạm chưa xử lý</Tag>}</div></div>
