...
export default function BaseInput({ className, ...props }: BaseInputProps) {
  return (
    <Input
      className={cn(
        "!rounded-[5px] !px-4 !h-10 !py-2 !shadow-none !bg-slate-50 hover:!bg-slate-100 focus:!bg-white !text-[14px] !text-gray-900 transition-all duration-300 [&:-webkit-autofill]:[transition:background-color_9999s_ease-in-out_0s] [&:-webkit-autofill]:[-webkit-text-fill-color:#0f172a]",
        className
      )}
